package rules

import (
	"database/sql"
	"encoding/json"
	"fmt"
	"time"

	"github.com/google/uuid"
)

// Store 规则存储
type Store struct {
	db *sql.DB
}

// NewStore 创建新的规则存储
func NewStore(db *sql.DB) *Store {
	return &Store{db: db}
}

// Init 初始化规则表
func (s *Store) Init() error {
	query := `
	CREATE TABLE IF NOT EXISTS rules (
		id VARCHAR(36) PRIMARY KEY,
		name VARCHAR(255),
		enabled BOOLEAN DEFAULT TRUE,
		priority INTEGER DEFAULT 0,
		match_config TEXT,
		action_config TEXT,
		created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
		updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
	);
	CREATE INDEX IF NOT EXISTS idx_rules_enabled ON rules(enabled);
	`
	_, err := s.db.Exec(query)
	return err
}

// Create 创建规则
func (s *Store) Create(rule *Rule) error {
	if rule.ID == "" {
		rule.ID = uuid.New().String()
	}

	now := time.Now()
	rule.CreatedAt = now
	rule.UpdatedAt = now

	matchJSON, err := json.Marshal(rule.Match)
	if err != nil {
		return fmt.Errorf("序列化匹配条件失败: %w", err)
	}

	actionJSON, err := json.Marshal(rule.Action)
	if err != nil {
		return fmt.Errorf("序列化动作失败: %w", err)
	}

	query := `
		INSERT INTO rules (id, name, enabled, priority, match_config, action_config, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`

	_, err = s.db.Exec(query,
		rule.ID, rule.Name, rule.Enabled, rule.Priority,
		string(matchJSON), string(actionJSON),
		rule.CreatedAt, rule.UpdatedAt,
	)

	return err
}

// GetByID 根据 ID 获取规则
func (s *Store) GetByID(id string) (*Rule, error) {
	query := `
		SELECT id, name, enabled, priority, match_config, action_config, created_at, updated_at
		FROM rules WHERE id = ?
	`

	rule := &Rule{}
	var matchJSON, actionJSON string

	err := s.db.QueryRow(query, id).Scan(
		&rule.ID, &rule.Name, &rule.Enabled, &rule.Priority,
		&matchJSON, &actionJSON,
		&rule.CreatedAt, &rule.UpdatedAt,
	)

	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("查询规则失败: %w", err)
	}

	if err := json.Unmarshal([]byte(matchJSON), &rule.Match); err != nil {
		return nil, fmt.Errorf("解析匹配条件失败: %w", err)
	}

	if err := json.Unmarshal([]byte(actionJSON), &rule.Action); err != nil {
		return nil, fmt.Errorf("解析动作失败: %w", err)
	}

	return rule, nil
}

// List 获取规则列表
func (s *Store) List() ([]*Rule, error) {
	query := `
		SELECT id, name, enabled, priority, match_config, action_config, created_at, updated_at
		FROM rules
		ORDER BY priority ASC, created_at DESC
	`

	rows, err := s.db.Query(query)
	if err != nil {
		return nil, fmt.Errorf("查询规则列表失败: %w", err)
	}
	defer rows.Close()

	var rules []*Rule
	for rows.Next() {
		rule := &Rule{}
		var matchJSON, actionJSON string

		err := rows.Scan(
			&rule.ID, &rule.Name, &rule.Enabled, &rule.Priority,
			&matchJSON, &actionJSON,
			&rule.CreatedAt, &rule.UpdatedAt,
		)
		if err != nil {
			return nil, fmt.Errorf("扫描规则失败: %w", err)
		}

		if err := json.Unmarshal([]byte(matchJSON), &rule.Match); err != nil {
			continue
		}
		if err := json.Unmarshal([]byte(actionJSON), &rule.Action); err != nil {
			continue
		}

		rules = append(rules, rule)
	}

	return rules, nil
}

// Update 更新规则
func (s *Store) Update(rule *Rule) error {
	rule.UpdatedAt = time.Now()

	matchJSON, err := json.Marshal(rule.Match)
	if err != nil {
		return fmt.Errorf("序列化匹配条件失败: %w", err)
	}

	actionJSON, err := json.Marshal(rule.Action)
	if err != nil {
		return fmt.Errorf("序列化动作失败: %w", err)
	}

	query := `
		UPDATE rules
		SET name = ?, enabled = ?, priority = ?, match_config = ?, action_config = ?, updated_at = ?
		WHERE id = ?
	`

	result, err := s.db.Exec(query,
		rule.Name, rule.Enabled, rule.Priority,
		string(matchJSON), string(actionJSON),
		rule.UpdatedAt, rule.ID,
	)
	if err != nil {
		return fmt.Errorf("更新规则失败: %w", err)
	}

	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("规则不存在")
	}

	return nil
}

// Delete 删除规则
func (s *Store) Delete(id string) error {
	query := "DELETE FROM rules WHERE id = ?"
	result, err := s.db.Exec(query, id)
	if err != nil {
		return fmt.Errorf("删除规则失败: %w", err)
	}

	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("规则不存在")
	}

	return nil
}

// Toggle 启用/禁用规则
func (s *Store) Toggle(id string) (bool, error) {
	// 先获取当前状态
	rule, err := s.GetByID(id)
	if err != nil {
		return false, err
	}
	if rule == nil {
		return false, fmt.Errorf("规则不存在")
	}

	// 切换状态
	rule.Enabled = !rule.Enabled
	rule.UpdatedAt = time.Now()

	query := "UPDATE rules SET enabled = ?, updated_at = ? WHERE id = ?"
	_, err = s.db.Exec(query, rule.Enabled, rule.UpdatedAt, id)
	if err != nil {
		return false, fmt.Errorf("切换规则状态失败: %w", err)
	}

	return rule.Enabled, nil
}

// GetEnabled 获取所有启用的规则
func (s *Store) GetEnabled() ([]*Rule, error) {
	query := `
		SELECT id, name, enabled, priority, match_config, action_config, created_at, updated_at
		FROM rules
		WHERE enabled = TRUE
		ORDER BY priority ASC
	`

	rows, err := s.db.Query(query)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	var rules []*Rule
	for rows.Next() {
		rule := &Rule{}
		var matchJSON, actionJSON string

		err := rows.Scan(
			&rule.ID, &rule.Name, &rule.Enabled, &rule.Priority,
			&matchJSON, &actionJSON,
			&rule.CreatedAt, &rule.UpdatedAt,
		)
		if err != nil {
			continue
		}

		json.Unmarshal([]byte(matchJSON), &rule.Match)
		json.Unmarshal([]byte(actionJSON), &rule.Action)

		rules = append(rules, rule)
	}

	return rules, nil
}

// Count 获取规则数量
func (s *Store) Count() (int, error) {
	var count int
	err := s.db.QueryRow("SELECT COUNT(*) FROM rules").Scan(&count)
	return count, err
}

// Reorder 重新排序规则
func (s *Store) Reorder(ids []string) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}

	for i, id := range ids {
		_, err := tx.Exec("UPDATE rules SET priority = ? WHERE id = ?", i, id)
		if err != nil {
			tx.Rollback()
			return err
		}
	}

	return tx.Commit()
}
