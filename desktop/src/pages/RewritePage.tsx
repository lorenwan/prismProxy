    ...
    <div className="flex h-full">
      ...
      <div className="w-80 border-r border-[#3b4261] flex flex-col bg-[#1a1b26]">
        ...
        <div className="flex-1 overflow-y-auto">
          {rules.map((rule) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-[#565f89]">{getTypeLabel(rule.type)}</div>
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation()
          ...
          {rules.length === 0 && (
            <div className="p-4 text-center text-[#565f89] text-sm">暂无重写规则</div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-[#24283b] overflow-y-auto">
        <div className="p-4 space-y-4 max-w-2xl">
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-[#7aa2f7]">规则类型</h3>
            <select
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-[#1a1b26] border border-[#3b4261] rounded text-sm h-32 resize-none focus:border-[#7aa2f7] focus:outline-none font-mono"
              placeholder={
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-[#24283b] border border-[#3b4261] rounded-lg p-4 w-80" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-[#565f89] mb-4">确定要删除规则 "{deleteConfirm.name}" 吗？此操作不可撤销。</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteConfirm(null)} className="px-3 py-1.5 text-sm bg-[#1a1b26] rounded hover:bg-[#3b4261]">
                取消
              </button>
              <button onClick={() => handleDelete(deleteConfirm)} className="px-3 py-1.5 text-sm bg-[#f7768e] text-[#1a1b26] rounded hover:bg-[#ff9eaf]">
                删除
              </button>
            </div>
          </div>
