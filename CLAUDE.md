...
### 目录规则
1. **components/ui/** - 只放 shadcn/ui 基础组件，不做业务定制
2. **components/layout/** - 只放全局布局组件
3. **features/** - 按业务功能拆分模块，每个模块独立
4. **pages/** - 页面组件作为路由入口，组合 features 组件
5. **services/** - 放置 Tauri IPC 调用，按模块命名
6. **hooks/** - 自定义 hooks，可跨模块复用
7. **lib/** - 工具函数，按模块拆分文件

### 组件规范
- 使用 shadcn/ui 组件，不要自定义基础 UI
- 同类 UI 组件出现超 2 次就封装复用
